...
export async function getContacts(query: any | null = null) {
...
export async function updateContact(id: any, updates: any) {
  await fakeNetwork();
  let contacts: any = await localforage.getItem("contacts");
  let contact: any = contacts.find((contact: any) => contact.id === id);
...
async function fakeNetwork(key: any | null = null) {
  if (!key) {
    fakeCache = {};
  }
